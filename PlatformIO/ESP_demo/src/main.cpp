...
#define CHAR_CNT_UUID "E0AB871B-6E89-45F7-8BF2-4F3978D1A2EB"
...
void setup() {
  ...
  // 创建一个 BLE 特征
  // 1. 读写特征值
  *pChar = BLECharacteristic(CHAR_UUID, BLECharacteristic::PROPERTY_READ |
                                            BLECharacteristic::PROPERTY_WRITE);
  pService->addCharacteristic(pChar);
  delay(100);
  // 2. 周期广播特征值
  *pCharCnt =
      BLECharacteristic(CHAR_CNT_UUID, BLECharacteristic::PROPERTY_READ |
                                           BLECharacteristic::PROPERTY_NOTIFY);
  pService->addCharacteristic(pCharCnt);
