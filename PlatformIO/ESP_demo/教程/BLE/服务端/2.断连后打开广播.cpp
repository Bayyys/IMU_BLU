...
void setup() {
  ...
  /**
   * 特征值的属性
   * BLECharacteristic::PROPERTY_READ    可读(读取特征值)
   * BLECharacteristic::PROPERTY_WRITE   可写(写入特征值[带响应])
   * BLECharacteristic::PROPERTY_NOTIFY  可通知(订阅特征值)
   * BLECharacteristic::PROPERTY_INDICATE 可指示(订阅特征值[带确认])
   * BLECharacteristic::PROPERTY_BROADCAST 可广播(广播特征值)
   * BLECharacteristic::PROPERTY_WRITE_NR  可写无响应(写入特征值[无响应])
   */
  // 设置特征值
